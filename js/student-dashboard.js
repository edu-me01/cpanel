...
class StudentDashboard {
  ...
  loadTasks() {
    ...
    // Store sample tasks in session if not exists
    if (!session.getItem("tasks")) {
      session.setItem("tasks", JSON.stringify(sampleTasks));
    }

    const tasks = JSON.parse(session.getItem("tasks") || "[]");
    const studentTasks = tasks.filter(
      (task) => task.assignedTo === "student-1"
    );
    ...
    studentTasks.forEach((task) => {
      ...
      tr.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-primary" onclick="studentDashboard.viewTask('${
                      task.id
                    }')">
    ...
    if (completedTasksElement) {
      completedTasksElement.textContent = studentTasks.filter(
        (t) => t.completed
      ).length;
    }
    ...
    if (upcomingTasksElement) {
      upcomingTasksElement.textContent = studentTasks.filter(
        (t) => !t.completed && this.isUpcoming(t.dueDate)
      ).length;
    }
  }
  ...
  loadSubmissions() {
    ...
    // Store sample submissions in session if not exists
    if (!session.getItem("submissions")) {
      session.setItem("submissions", JSON.stringify(sampleSubmissions));
    }

    const submissions = JSON.parse(session.getItem("submissions") || "[]");
    const studentSubmissions = submissions.filter(
      (sub) => sub.studentId === "student-1"
    );
  ...
  loadProgressData() {
    // Get tasks and submissions from session
    const tasks = JSON.parse(session.getItem("tasks") || "[]");
    const studentTasks = tasks.filter(
      (task) => task.assignedTo === "student-1"
    );
    ...
    // Store sample attendance in session if not exists
    if (!session.getItem("attendance")) {
      session.setItem("attendance", JSON.stringify(sampleAttendance));
    }
      ...
      attendanceHistory.innerHTML = studentAttendance
        ...
        .map(
          (a) => `
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        ${a.date}
                        <span class="badge bg-${this.getAttendanceColor(
                          a.status
                        )}">
                            ${a.status}
                        </span>
  ...
  loadAttendanceData() {
    ...
    const attendanceStatus = session.getItem("attendanceOpen") === "true";
    ...
    if (statusElement) {
      statusElement.className = `badge bg-${
        attendanceStatus ? "success" : "danger"
      }`;
  ...
  addEventListeners() {
    ...
    // Listen for session changes (for cross-tab communication)
  ...
  completeTask(taskId) {
    ...
    if (task) {
      task.completed = true;
      session.setItem("tasks", JSON.stringify(tasks));
      this.loadTasks();
      this.loadProgressData();
